...
def concatenate_input_noise_map(input, noise_sigma):
    ...
    downsampledfeatures = torch.zeros(N, Cout, Hout, Wout, device=device, dtype=dtype)
...
class UpSampleFeaturesFunction(Function):
    ...
    def forward(ctx, input):
        ...
        result = torch.zeros(N, Cout, Hout, Wout, device=device, dtype=dtype)
        for idx in range(sca2):
    ...
    def backward(ctx, grad_output):
        ...
        # Build output
        grad_input = torch.zeros(N, Cg_in, Hg_in, Wg_in, device=device, dtype=dtype)
        # Populate output
        for idx in range(sca2):
